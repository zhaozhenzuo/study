...
/**
 * 解决问题：<br/>
 * 从开始字符串，每次变换一个字符，最后变成结束字符串。求这个最短变化路径。<br/>
 * 思路：<br/>
 * 将单词放入queue，处理这个单词，将所有可能并且在dict集合中的单词放入queue。然后每个新单词需要记录下父单词结点
 * 
 * @author zhaozhenzuo
 *
 */
public class WordLadder2Performance {
	...
	private Map<String, List<Node>> resMap = new HashMap<String, List<Node>>();

	private Map<String, Node> visitedMap = new HashMap<String, Node>();

	Map<String, Set<String>> wordCanTransverMap = new HashMap<String, Set<String>>();

	public List<List<String>> findLadders(String start, String end, List<String> dictParam) {
		
		//list to set
		Set<String> dict=new HashSet<String>();
		dict.addAll(dictParam);

		this.bpProcess(start, end, dict);

		List<List<String>> resLadders = new ArrayList<List<String>>();

		List<Node> resList = this.resMap.get(end);
		if (resList == null) {
			return resLadders;
		}

		for (Node node : resList) {
			List<String> tempList = this.dpIterateNode(node);
			if (tempList != null) {
				resLadders.add(tempList);
			}
		}
	...
	private List<String> dpIterateNode(Node node) {
		if (node == null) {
			return null;
		}
	 ...
	 * 广度优先处理
	 */
	private void bpProcess(String start, String end, Set<String> dict) {
		if (start == null || end == null || dict == null) {
			throw new IllegalArgumentException("开始，结束单词及字典均不能为空");
		}
		
		/**
		 * 首结点
		 */
		Node firstNode = new Node(null, start, 0);

		List<Node> tempList = new ArrayList<Node>();
		tempList.add(firstNode);
		resMap.put(start, tempList);

		Queue<Node> queue = new LinkedList<Node>();
		queue.add(firstNode);

		boolean visitedEndFlag = false;
		
		long oldTimeForBuild=System.currentTimeMillis();
		
		int length=start.length();
		
		// 先为字典集合，包括首单词建立可转换map
		StringBuilder builder=new StringBuilder(length);
		Set<String> canTransferListForStart = this.getCanTransferWordList(start,length,dict,builder);
		wordCanTransverMap.put(start, canTransferListForStart);

		for (String tempWord : dict) {
			Set<String> canTransSet = this.getCanTransferWordList(tempWord,length,dict,builder);
			wordCanTransverMap.put(tempWord, canTransSet);
		}

		long costForBuild=System.currentTimeMillis()-oldTimeForBuild;
		System.out.println("costForBuild:"+costForBuild);
		
		while (!queue.isEmpty()) {
			Node wordNode = queue.poll();
			int level = wordNode.getLevel();
			if (visitedEndFlag) {
				int firstEndNodeLevel = resMap.get(end).get(0).getLevel();
				if (level > firstEndNodeLevel) {
					break;
				}
			}

			String word = wordNode.getData();
			Set<String> canTransferWordSet = wordCanTransverMap.get(word);
			int levenOfNewWord = level + 1;
			for (String newWord : canTransferWordSet) {
				/**
				 * 是否应该加入到处理结果中<br/>
				 * 要求满足：<br/>
				 * 1.没有处理过，即不在resMap<br/>
				 * 2.在dict中存在
				 */
				visitedEndFlag = this.processNewWord(newWord, word, end, queue, wordNode, levenOfNewWord, dict);
			}
		}
		
	}

	private Set<String> getCanTransferWordList(String word,int length,Set<String> dict,StringBuilder builder) {
		...
		for (int i = 0; i < word.length(); i++) {
			...
			/**
			 * 这个单词每个位置都会发生变化，依次从第一个字符开始变化
			 */
			char posChar = word.charAt(i);
	...
	private boolean processNewWord(String newWord, String word, String end, Queue<Node> queue, Node wordNode,
			int levenOfNewWord, Set<String> dict) {
		boolean visitedEndFlag = false;
		if (this.newWordShouldProcess(newWord, levenOfNewWord, dict)) {
			Node node = new Node(wordNode, newWord, levenOfNewWord);
			this.addNodeToResMap(newWord, node);
			visitedMap.put(newWord, node);
		...
		return visitedEndFlag;
	}

	private boolean newWordShouldProcess(String newWord, int level, Set<String> dict) {
		Node visitedNode = visitedMap.get(newWord);
		if (visitedNode != null && level > visitedNode.getLevel()) {
			/**
			 * 不同层的单词不允许出现重复访问
			 */
			return false;
	...
	private void addNodeToResMap(String word, Node node) {
		List<Node> nodeExistList = resMap.get(word);
		if (nodeExistList == null) {
			nodeExistList = new ArrayList<Node>();
			resMap.put(word, nodeExistList);
		}

		nodeExistList.add(node);
	}
	...
	class Node {
		...
		public Node(Node parent, String data, Integer level) {
			this.parent = parent;
			this.data = data;
			this.level = level;
		}
